...
class FamilyMemberRepository implements FamilyMemberRepositoryInterface
{
    public function getAll(
        ...
        $query = FamilyMember::query()->where(function ($query) use ($search) {
            if ($search) {
                $query->search($search);
            }
        });
    ...
    public function getById(
        ...
        $query = FamilyMember::query()->where('id', $id)->with('headOfFamily');
    ...
    public function update(
        ...
        DB::beginTransaction();
        try {
            $familyMember = FamilyMember::query()->find($id);

            if (!$familyMember) {
                throw new Exception('Data Anggota Keluarga Tidak Ditemukan');
            }
